(function($) {
    ...
    HT.visitRequest = () => {
        $('.hp-visit-form').on('submit', function(e){
            e.preventDefault()
            let _this = $(this)
            let formData = new FormData(this)
            let submitBtn = _this.find('button[type="submit"]')
            ...
            $.ajax({
                ...
                dataType: 'json', 
                beforeSend: function() {
                    submitBtn.html('<i class="fa fa-spinner fa-spin"></i> Đang gửi...').attr('disabled', true)
                },
                success: function(res) {
                    submitBtn.html(originalText).removeAttr('disabled')
                    if(res.success === true){
                        toastr.success(res.message || 'Gửi yêu cầu thành công!', 'Thông báo')
                        _this[0].reset()
                    } else {
                        toastr.error(res.message || 'Có lỗi xảy ra, vui lòng thử lại.', 'Lỗi')
                    }
                },
                error: function(jqXHR) {
                    submitBtn.html(originalText).removeAttr('disabled')
                    let errorMsg = 'Có lỗi xảy ra trong quá trình gửi.';
