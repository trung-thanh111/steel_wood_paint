    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-center js-switch-{{ $slide->id }}">
                <input type="checkbox" value="{{ $slide->publish }}" class="js-switch status " data-field="publish" data-model="{{ $config['model'] }}" data-namespace="Core" {{ ($slide->publish == 2) ? 'checked' : '' }} data-modelId="{{ $slide->id }}" />
            </td>
